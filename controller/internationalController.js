...
const addinternationalfaculty = async function(req, res) {
    try {
        const {
            university,
            faculty,
        ...
        const newfaculty = new international({
            university,
            faculty,
            normal_or_Dual,
            programs,
...
const deletefaculty = async function(req, res) {
    try {
        ...
        const result = await international.deleteOne({ faculty: name });
...
const getAllfaculty = async function(req,res){
    try{
        const faculty = await international.find();
        if(faculty.length === 0){
...
module.exports = { addinternationalfaculty, deletefaculty, getAllfaculty, };
