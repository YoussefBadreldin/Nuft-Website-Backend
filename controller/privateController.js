...
const addprivatefaculty = async function(req, res) {
    try {
        const {
            university,
            faculty,
...
const deletefaculty = async function(req, res) {
    try {
        ...
        const result = await private.deleteOne({ faculty: name });
...
const getAllfaculty = async function(req,res){
    try{
        const faculty = await private.find();
        if(faculty.length === 0){
...
module.exports = { addprivatefaculty, deletefaculty, getAllfaculty, };
